...
namespace std {
    ...
    [[noreturn]] void throw_with_nested(T&& t) {
        ...
        if constexpr (is_class_v<U> && !is_final_v<U> && !is_base_of_v<nested_exception, U>) {
            struct throwable : public U, nested_exception {
            public:
                inline explicit throwable(const U& u) : U(u), nested_exception() {}
            };

            throw throwable(__internal::forward<T>(t));
        } else {
