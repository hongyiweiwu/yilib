...
namespace std {
    ...
    struct default_delete<T[]> {
        ...
        template<class U>
        requires __internal::is_complete<U>::value && is_convertible_v<U*, T*>
    ...
    namespace __internal {
        ...
        struct __unique_ptr_pointer_type {
            using type = T*;
        };

        template<class T, class D>
        requires requires { typename remove_reference_t<D>::pointer; }
        struct __unique_ptr_pointer_type<T, D> {
    ...
    public:
        ...
        requires requires { get_deleter()(get()); } {
            pointer old = get();
            this->p = p;
            if (old != nullptr) {
                get_deleter()(get());
        ...
        requires is_nothrow_swappable_v<decltype(get_deleter())> {
            using std::swap;
        ...
        requires (!is_pointer_v<deleter_type>) && is_nothrow_default_constructible_v<deleter_type> = default;
        ...
        requires (!is_pointer_v<deleter_type>) && is_nothrow_default_constructible_v<deleter_type>
            : p(p), d() {}

        unique_ptr(type_identity_t<pointer> p, const D& d) noexcept
        requires is_reference_v<D> || is_nothrow_copy_constructible_v<D> : p(p), d(d) {}

        unique_ptr(type_identity_t<pointer> p, remove_reference_t<D>&& d)
        requires is_reference_v<D> = delete;

        unique_ptr(type_identity_t<pointer> p, D&& d) noexcept
        requires (!is_reference_v<D>) && is_nothrow_move_constructible_v<D> : p(p), d(move(d)) {}

        unique_ptr(unique_ptr&& u) noexcept
        requires is_move_constructible_v<D> && (is_reference_v<D> || is_nothrow_move_constructible_v<D>)
        ...
        requires (!is_pointer_v<deleter_type>) && is_default_constructible_v<deleter_type>
            : unique_ptr() {}
    ...
    public:
        ...
        template<class U>
        requires is_same_v<U, pointer> || (is_same_v<pointer, element_type*> && is_pointer_v<U> && is_convertible_v<U, element_type*>)
        ...
        template<class U>
        requires is_reference_v<D> && is_nothrow_copy_constructible_v<D>
        ...
        template<class U>
        requires (!is_reference_v<D>) && is_nothrow_move_constructible_v<D>
            && (is_same_v<U, pointer> || is_null_pointer_v<U> || (is_same_v<pointer, element_type*> && is_pointer_v<U> && is_convertible_v<U, element_type*>))
