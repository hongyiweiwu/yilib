...
namespace std {
    ...
    namespace __internal {
        template<class T, T N, T Curr, T ...Remaining>
        constexpr auto __make_integer_sequence_test() {
            if constexpr (N == 0) {
        ...
        template<class T, class IntConstant>
        requires (IntConstant::value >= 0)
    ...
    requires is_integral_v<T> && (N >= 0)
    using make_integer_sequence = typename __internal::__make_integer_sequence<T, integral_constant<T, N>>::type;
    ...
    private:
        ...
        template<class T>
        static constexpr bool is_implicit_default_constructible = requires (void (&fn)(T)) { fn({}); };
    public:
        using first_type = T1;
        ...
        constexpr explicit(!is_implicit_default_constructible<T1> ||is_implicit_default_constructible<T2>)
        pair()
        ...
        template<class U1, class U2>
        requires is_constructible_v<T1, const T1&> && is_constructible_v<T2, const T2&>
...
public:
        ...
        requires is_constructible_v<T1, Args1...> && is_constructible_v<T2, Args2...>
    ...
    template<class T1, class T2>
    constexpr common_comparison_category_t<__internal::synth_three_way_result<T1>, __internal::synth_three_way_result<T2>>
    operator<=>(const pair<T1, T2>& x, const pair<T1, T2>& y) {
        if (const auto c = __internal::synth_three_way(x.first, y.first); c != 0) {
            return c;
        }
