...
namespace std {
    ...
    concept convertible_to = is_convertible_v<From, To> && requires (add_rvalue_reference_t<From> (&f)()) { static_cast<To>(f()); };
    ...
    template<class T, class U>
    concept common_with = same_as<common_type_t<T, U>, common_type_t<U, T>> && requires {
    ...
    concept assignable_from = is_lvalue_reference_v<LHS> && common_reference_with<const remove_reference_t<LHS>&, const remove_reference_t<RHS>&>
        && requires (LHS lhs, RHS&& rhs) { { lhs = __internal::forward<RHS>(rhs) } -> same_as<LHS>; };
    ...
    namespace __internal {
        template<class T>
        concept boolean_testable = convertible_to<T, bool> && requires (T&& t) { { !forward<T>(t) } -> convertible_to<bool>; };
    ...
    template<class T>
    concept totally_ordered = equality_comparable<T> && __internal::partially_ordered_with<T, T>;
    ...
    template<class F, class ...Args>
    concept predicate = regular_invocable<F, Args...> && __internal::boolean_testable<invoke_result_t<F, Args...>>;

    template<class R, class T, class U>
    concept relation = predicate<R, T, T> && predicate<R, T, U> && predicate<R, U, U> && predicate<R, U, T>;
