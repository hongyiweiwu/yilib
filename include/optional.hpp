...
namespace std {
    ...
    private:
        struct storage {
            ...
            explicit constexpr storage(no_val_t) noexcept : has_value(false), dummy() {}
    ...
    public:
        ...
        constexpr optional() noexcept : val(storage::no_val) {};
        constexpr optional(nullopt_t) noexcept : val(storage::no_val) {};

        constexpr optional(const optional& rhs)
        requires is_copy_constructible_v<T> && is_trivially_copy_constructible_v<T> = default;
        ...
        requires is_move_constructible_v<T> && is_trivially_move_constructible_v<T> = default;

        constexpr optional(optional&& rhs) noexcept(noexcept(T(move(*rhs))))
        ...
        template<class U = T>
        requires is_constructible_v<T, U> && (!is_same_v<remove_cvref_t<U>, in_place_t>) && (!is_same_v<remove_cvref_t<U>, optional>)
        constexpr explicit(!is_convertible_v<U, T>) optional(U&& v) : val(forward<U>(v)) {}

        template<class U>
        requires is_constructible_v<T, const U&> && (!is_constructible_v<T, optional<U>&>)
        ...
        template<class U>
        requires is_constructible_v<T, U> && (!is_constructible_v<T, optional<U>&>)
            && (!is_constructible_v<T, optional<U>&&>) && (!is_constructible_v<T, const optional<U>&>)
            && (!is_constructible_v<T, const optional<U>&&>) && (!is_convertible_v<optional<U>&, T>)
            && (!is_convertible_v<optional<U>&&, T>) && (!is_convertible_v<const optional<U>&, T>)
            && (!is_convertible_v<const optional<U>&&, T>)
        ...
        requires (is_trivially_copy_constructible_v<T> && is_trivially_copy_assignable_v<T> && is_trivially_destructible_v<T>) = default;

        constexpr optional& operator=(const optional& rhs)
        requires (!is_trivially_copy_constructible_v<T>) || (!is_trivially_copy_assignable_v<T>) || (!is_trivially_destructible_v<T>) {
            if (*this && rhs) {
                val.val = *rhs;
            } else if (*this && !rhs) {
                val.reset();
        ...
        requires (is_trivially_move_constructible_v<T> && is_trivially_move_assignable_v<T> && is_trivially_destructible_v<T>) = default;
        ...
        constexpr const T& value() const& {
            return bool(*this) ? val.val : throw bad_optional_access();
        }
        ...
        constexpr T&& value() && {
            return bool(*this) ? move(val.val) : throw bad_optional_access();
        }
        ...
        constexpr T value_or(U&& v) const& {
            return bool(*this) ? val.val : static_cast<T>(forward<U>(v));
        }
    ...
    requires requires (const optional<T>& x, const optional<T>& y) { {*x == *y} -> convertible_to<bool>; }
    ...
    template<class T, class U>
    requires requires (const optional<T>& x, const U& v) { {*x == v} -> convertible_to<bool>; }
    constexpr bool operator==(const optional<T>& x, const U& v) {
        return bool(x) ? *x == v : false;
    }

    template<class T, class U>
    requires requires (const optional<T>& x, const U& v) { {v == *x} -> convertible_to<bool>; }
    ...
    struct hash<optional<T>> : hash<__internal::__enabled_hash_t> {
        std::size_t operator()(const optional<T>& key) const {
